...
app = Flask(__name__)
CORS(app)  # Cho phép CORS cho mọi domain (localhost, flutter app,...)
...
def api_get_users():
    users = list(collection.find())
    result = [{"name": user["name"], "image_url": url_for('known_faces', filename=user["filename"], _external=True)} for user in users]
